...
import { loadAiSettings, saveAiSettings, clearAiSettings, getAiReadyState } from "./ai/providers.js";
...
function loadOverlays() {
  try {
    ...
    return {
      ...
      learning: obj.learning && typeof obj.learning === "object" ? { ...defaultOverlays().learning, ...obj.learning } : defaultOverlays().learning
    };
  } catch {
...
function buildTaskList(view) {
  ...
  if (view === "week") {
    ...
    // Today view is special: we will section it (Overdue, Today, Next)
    // so filtering happens later.
  }
...
function attachLongPress(el, onLongPress) {
  ...
  const start = () => {
    moved = false;
    timer = setTimeout(() => { if (!moved) onLongPress(); }, 420);
  };
...
function renderCard(t) {
  ...
  box.addEventListener("click", (e) => { e.stopPropagation(); toggleComplete(t.id); });
...
function renderTodaySections(listEl) {
  ...
  const dueToday = filtered.filter(t => !isOverdue(t, today)).filter(t => {
    const d = parseISODate(t.due_date) || parseISODate(t.start_date);
    return d && isoFromDate(d) === isoFromDate(today);
  });
  ...
  const makeSection = (label, arr) => {
    ...
    if (state.zen) {
      // Zen: show just first item, calm
      listEl.appendChild(renderCard(arr[0]));
    } else {
...
function render() {
  ...
  // Other views: grouped lightly by date
...
function initFilterOptions() {
  ...
  document.getElementById("filterMonth").innerHTML =
    `<option value="any">All Months</option>` + months.map(m => `<option value="${m}">${m}</option>`).join("");
}
...
/* ---------- Export / Import ---------- */
function exportOverlays() { dlFile("overrides.json", JSON.stringify(state.overlays, null, 2)); }
...
function backupMerged() { dlFile("merged.json", JSON.stringify(state.merged, null, 2)); }
...
function renderAiHint() {
  ...
  // If enabled but no pending payload, keep row hidden for clean UI
  row.hidden = true;
}
...
function showAiReview(payload) {
  ...
  list.innerHTML = (payload.ops || []).map(o => `
    <div class="ai-op">
      ...
      ${o.fields ? `<pre class="muted small" style="margin:8px 0 0;white-space:pre-wrap">${safeText(JSON.stringify(o.fields, null, 2))}</pre>` : ""}
    </div>
...
function applyAiOps(payload) {
  ...
  for (const o of ops) {
    if (o.op === "add") {
      const id = (o.id && o.id.startsWith("temp_")) ? o.id : `temp_${Date.now().toString(36)}_${Math.random().toString(36).slice(2,6)}`;
    ...
    if (o.op === "update") Object.assign(ensurePatch(o.id), o.fields || {});
...
async function aiRefineTaskById(id, { origin = "manual" } = {}) {
  ...
  // Auto-open review (but disappears after apply/discard)
...
function wireUI() {
  ...
  document.getElementById("btnToggleFilters").addEventListener("click", () => { filterbar.hidden = !filterbar.hidden; });
  ...
  document.getElementById("btnActionToday").addEventListener("click", () => { if (!state.actionId) return; moveToToday(state.actionId, "longpress_today"); closeSheet("actionSheet"); });
  document.getElementById("btnActionDefer").addEventListener("click", () => { if (!state.actionId) return; deferOneDay(state.actionId); closeSheet("actionSheet"); });
  document.getElementById("btnActionAiRefine").addEventListener("click", async () => { if (!state.actionId) return; closeSheet("actionSheet"); await aiRefineTaskById(state.actionId, { origin: "longpress_refine" }); });
  ...
  inboxInput.addEventListener("keydown", (e) => { if (e.key === "Enter") document.getElementById("btnInboxAdd").click(); });
  ...
  document.getElementById("btnAiSaveSettings").addEventListener("click", () => {
    ...
    saveAiSettings({ enabled, autoRefine, provider, model, endpoint, headersJson, rememberKey, apiKey: rememberKey ? apiKey : "" });
